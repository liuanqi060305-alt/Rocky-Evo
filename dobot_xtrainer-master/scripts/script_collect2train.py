...
def load_data(one_dataset_dir):
    ...
    for i in range(len(data_pose_list)):
        with open(data_pose_list[i], "rb") as f:
            ...
            # V2 起不再导出 joint_velocities —— 它在 V1 里就只是 joint_positions
            # 的副本（dobot.py:200 同一变量，实测 494/494 帧相等），不含速度信息。
            # 这里保持 HDF5 的 qvel 字段存在以兼容既有 ACT 训练脚本，取值沿用
            # V1 的实际内容（= qpos），语义上并未变化，只是不再假装它是速度。
            qvel.append(data_single.get('joint_velocities',
                                        data_single['joint_positions']))
