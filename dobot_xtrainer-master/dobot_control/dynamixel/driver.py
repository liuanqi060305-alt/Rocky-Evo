...
# Retry budget for the register writes done during init. The bus is shared with the
# adapter board's key protocol, so a single corrupt reply is normal and recoverable.
IO_RETRIES = 5
IO_RETRY_DELAY = 0.05

# /dev/ttyACM* numbering follows USB enumeration order, so it can swap between boots
# or replugs. Ports claimed by this process are tracked so probing never steals the
# port belonging to the other hand.
_claimed_ports = set()
_claim_lock = Lock()


def _ids_on_port(port: str, baudrate: int):
    """Return the set of Dynamixel IDs answering a broadcast ping on `port`."""
    ...
    try:
        ...
        found = set()
        # A single broadcast can be clipped when the bus is busy; two passes is plenty.
        for _ in range(2):
            handler.clearPort()
            data, comm_result = packet_handler.broadcastPing(handler)
            if comm_result == COMM_SUCCESS or data:
                found.update(data.keys())
            if found:
                break
            time.sleep(IO_RETRY_DELAY)
        return found
    except Exception:
        return set()
    finally:
...
def resolve_port(preferred_port: str, expected_ids: Sequence[int], baudrate: int) -> str:
    """Find the port that actually hosts `expected_ids`.

    Tries `preferred_port` first, then any other same-family serial port. Returns the
    preferred port unchanged if nothing better is found, so the caller still produces
    its normal diagnostics.
    """
    expected = set(expected_ids)
    prefix = "ttyACM" if "ttyACM" in preferred_port else "ttyUSB"

    candidates = [preferred_port]
    for entry in sorted(glob.glob(f"/dev/{prefix}*")):
    ...
    with _claim_lock:
        for port in candidates:
            if port in _claimed_ports:
                continue
            found = _ids_on_port(port, baudrate)
            if expected.issubset(found):
                if port != preferred_port:
                    print(
                        f"[dynamixel] {preferred_port} does not host IDs {sorted(expected)}; "
                        f"using {port} instead (USB enumeration order changed)."
                    )
                _claimed_ports.add(port)
                return port

        _claimed_ports.add(preferred_port)
...
class DynamixelDriver(DynamixelDriverProtocol):
    def __init__(
        ...
        # ttyACM numbering is not stable across replugs; find the port hosting our IDs.
        port = resolve_port(port, tuple(ids) + (append_id,), baudrate)
        self._port = port
        ...
        # Make sure every servo actually answers before writing any register. This
        # also drains leftover bytes from a previous run that died mid-transfer.
        self._wait_for_bus(port)
    ...
    def _wait_for_bus(self, port: str, timeout: float = 5.0):
        """Ping every servo until all of them answer, or raise with a useful message."""
        ...
        while pending and time.time() < deadline:
            ...
            still_pending = []
            for dxl_id in pending:
                _, comm_result, _ = self._packetHandler.ping(self._portHandler, dxl_id)
                if comm_result != COMM_SUCCESS:
                    last_error[dxl_id] = self._packetHandler.getTxRxResult(comm_result)
                    still_pending.append(dxl_id)
            pending = still_pending
            if pending:
                time.sleep(IO_RETRY_DELAY)

        if pending:
            details = ", ".join(f"ID {i} ({last_error.get(i, 'no reply')})" for i in pending)
            raise RuntimeError(
                f"No response from Dynamixel(s) on {port}: {details}. "
                f"Check that the arm is powered on, the USB cable is seated, and that no "
                f"other process is holding {port}."
            )

    def _write_register(self, dxl_id: int, addr: int, value: int, num_bytes: int):
        """Write a register, retrying transient bus errors. Caller must hold the lock."""
        writer = (
            self._packetHandler.write1ByteTxRx if num_bytes == 1
            else self._packetHandler.write2ByteTxRx
        )
        comm_result = None
        dxl_error = 0
        for attempt in range(IO_RETRIES):
            comm_result, dxl_error = writer(self._portHandler, dxl_id, addr, value)
            if comm_result == COMM_SUCCESS and dxl_error == 0:
                return
            # Drop any partial/garbage reply so the next attempt starts clean.
            self._portHandler.clearPort()
            if attempt < IO_RETRIES - 1:
                time.sleep(IO_RETRY_DELAY)

        raise RuntimeError(
            f"Failed to write addr {addr} on Dynamixel ID {dxl_id} after {IO_RETRIES} "
            f"attempts: comm={self._packetHandler.getTxRxResult(comm_result)}, "
            f"hardware error={self._packetHandler.getRxPacketError(dxl_error)}"
        )
    ...
    def set_torque_mode(self, enable: bool):
        ...
        with self._lock:
            for dxl_id in id_all:
                self._write_register(dxl_id, ADDR_TORQUE_ENABLE, torque_value, 1)
            ...
            with self._lock:
                ...
                for i, dxl_id in enumerate(self._ids):
                    if self._groupSyncRead.isAvailable(
                        ...
                        # Never raise here: this runs in a daemon thread, and an
                        # exception would kill it silently and hang get_joints()
                        # forever. Skip the frame and read again.
                        print(f"warning, no data for Dynamixel ID {dxl_id}, skipping frame")
                        incomplete = True
                        break
                if incomplete:
                    continue
    ...
    def close(self):
        ...
        release_port(self._port)

    def get_keys(self) -> np.ndarray:
        # Return a copy of the key array to avoid race conditions
        # _joint_keys starts as np.zeros(3) and is updated by _read_joint_angles
        # If _internal_getKey fails (returns []), we keep the last valid value
        with self._lock:
            key = self._joint_keys.copy() if len(self._joint_keys) == 3 else np.array([1, 1, 0])
    ...
    def set_joint_arg_2Byte(self, _id, addr, val):
        with self._lock:
            self._write_register(_id, addr, val, 2)
